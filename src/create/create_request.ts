import { FormattedOracleRequest, OracleRequest, RawOracleRequest } from "../types";
...
function _validateEthApiMethod(method: string) {
    if (method !== "eth_call") throw new Error("Only eth_call is currently supported");
}
...
export default async function createRequest(request: OracleRequest) {
    ...
    if (request.type === "raw") {
        ...
        if (rawRequest.rType === "eth") {
            ...
            finalStringRequest = `{"cid":${rawRequest.cid},"uri":${rawRequest.uri},"jsps":${_formatJsps(rawRequest.jsps)},${_addTrims(rawRequest.trims)}${_addPost(rawRequest.post)}"encoding":${rawRequest.encoding},"time":${rawRequest.time},"pow":${rawRequest.pow}}`;
        }
        ...
        if (formattedRequest.fType === "eth") {
            ...
            finalStringRequest = `{"cid":1,"uri":${formattedRequest.uri},"jsps":${_formatJsps(formattedRequest.jsps)},"params":[{"from":${formattedRequest.ethApi.params.from},"to":${formattedRequest.ethApi.params},"data":${formattedRequest.ethApi.params.data},"gas":${formattedRequest.ethApi.params.gas}, "latest"]},"encoding":"json"}`;
        } else {
            finalStringRequest = `{"cid":1,"uri":${formattedRequest.uri},"jsps":${_formatJsps(formattedRequest.jsps)}},${_addTrims(formattedRequest.trims)}${_addPost(formattedRequest.post)}"encoding":"json"}`;
        }
        finalStringRequest = await proofOfWork(finalStringRequest, getUTCTimestamp());
    } else {
